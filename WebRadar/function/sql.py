...
class DataProcessing:
    ...
    def get_data_by_column(self, table_name, column_name):
        """
        从指定表格的指定列中检索数据

        Args:
        - table_name: 表格名称
        - column_name: 要检索数据的列名称
        ...
        try:
            with self.db.connect() as conn:
                with conn.cursor() as cursor:
                    ...
                    if rows:
                        data = []
                        print(f"Printing data for column '{column_name}' in table '{table_name}' with corresponding IDs:")
                        for row in rows:
                            print(f"ID: {row[0]}, {column_name}: {row[1]}")
                            data.append({"id": row[0], column_name: row[1]})
                        return data
                    else:
    ...
    def stream_data_from_database(self, table_name, column_names):
        """
        从指定表格的指定列中流式传输数据

        Args:
        - table_name: 表格名称
        - column_names: 要流式传输数据的列名称列表
...
if __name__ == '__main__':
    ...
    #data_processor.get_data_by_column(table_name='radar_data', column_name='distance')
    for data in data_processor.stream_data_from_database(table_name='radar_data', column_names=['distance', 'angle']):
        print(data)
